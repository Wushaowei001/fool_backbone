...
Socket.prototype.send = function(data){
    ...
    try{
        ...
        if (global.logLevel > 1) util.log('error;', 'Socket.sendMessage',this.id, e, data);
    }
};
...
Socket.prototype.ping = function(){
    ...
    try {
        ...
        if (global.logLevel > 1) util.log('error;', 'Socket.ping send error', e, this.id);
    }
...
Socket.prototype.onClose = function(code){
    if (this.closed){
        if (global.logLevel > 1) util.log('log;','socket ', this.id,' already closed, new close code:', code);
        return;
    }
    if (this.ws && global.logLevel > 2) util.log('log;', 'closing socket', this.id, ' ws.state:', this.ws.readyState);
    if (global.logLevel > 1) util.log('log;','ws close, code:', code);
    this.closed = true;
    this.leaveAll();
...
Socket.prototype.readCookies = function(){
    ...
    try{
        ...
        if (global.logLevel > 3) util.log('log;', this.id, this.cookie);
    } catch (e){
...
Socket.prototype.enterRoom = function(room){
    if (this.closed){
        if (global.logLevel > 2) util.log('warn;', 'enter room closed socket', room, this.id);
        return;
    }
    if (!room) return;
